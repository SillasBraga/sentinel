...
export async function finishSOS(input: { id: string; finalIntensity: number; environment: string; strategies: string[]; durationSeconds: number }): Promise<ActionResult<{ awardedXp: boolean }>> {
  ...
  return { success: true, data: { awardedXp: xp.awarded } };
}
