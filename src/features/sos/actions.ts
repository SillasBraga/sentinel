...
export async function finishSOS(input: { id: string; finalIntensity: number; environment: string; strategies: string[]; durationSeconds: number }): Promise<ActionResult<{ awardedXp: boolean; levelUp: boolean; level: number; milestoneIds: PrivateMilestoneId[] }>> {
  ...
  return { success: true, data: { awardedXp: xp.awarded, levelUp: xp.levelUp, level: xp.level, milestoneIds: milestones.success ? milestones.milestoneIds : [] } };
}
