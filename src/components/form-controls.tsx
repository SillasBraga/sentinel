...
export function TextAreaField({ label, name, placeholder, required = false }: { label: string; name: string; placeholder?: string; required?: boolean }) { return <label className="grid gap-2 text-sm font-bold">{label}<textarea required={required} name={name} rows={4} placeholder={placeholder} className="resize-none rounded-2xl border border-[var(--line)] bg-white p-4 font-normal leading-7" /></label>; }
