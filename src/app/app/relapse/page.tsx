...
export default async function RelapsePage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <div className="mx-auto max-w-3xl p-5 py-8 sm:p-8 lg:p-12">
    ...
    <form action={createRelapse} className="mt-6 grid gap-6 rounded-[1.8rem] bg-white p-6 sm:p-8">
      ...
      <TextAreaField label="O que preparou esse momento? (opcional)" name="trigger" />
      <label className="grid gap-2 text-sm font-bold">Contexto (opcional)<input name="context" placeholder="Ex.: sozinho, tarde da noite" className="min-h-13 rounded-2xl border border-[var(--line)] px-4 font-normal" /></label>
      <TextAreaField label="O que você aprendeu? (opcional)" name="learning" />
      <TextAreaField label="Qual é o próximo passo pequeno? (opcional)" name="nextStep" />
      ...
      <SubmitButton><span className="inline-flex items-center justify-center gap-2"><RotateCcw size={18} /> Confirmar recaída e recomeçar</span></SubmitButton>
    </form>
  </div>;
}
